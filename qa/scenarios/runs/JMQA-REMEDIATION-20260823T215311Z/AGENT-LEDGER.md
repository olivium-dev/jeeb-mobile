...
| Android release artifact | Signing, forced-clean AAB build and inspection | HISTORICAL ARTIFACT PASS / UPLOAD HELD | SHA-256 `4586571b76cd6f952bfa037d04f712076bbfe693ae4c2be70f47c534c48bf62b`; predates reconstructed head and must be rebuilt after exact-head CI/review |
| iOS Maps/signing | Protected Maps key, App ID capabilities, forced-clean archive/export/validation | HISTORICAL ARTIFACT PASS / UPLOAD HELD | SHA-256 `eeebd0e2fa25aca08a78b308546660f2305ec2156c6f589ed75c08c8e6e7ef94`; predates reconstructed head and must be rebuilt after exact-head CI/review |
| Mobile source regression | Exact source suite, credential/WSS boundary, analyzer, protected-config, and workflow contracts | LOCAL COMPLETE / FULL CI PENDING | Source commit `e07d4542`: 7,882 passed, 66 intentionally skipped, 0 failed; 22 focused security tests and focused analyzer green; prior release/config gates remain green |
| Mobile source lineage | Preserve historical audit, freeze coherent revision, reconcile main, obtain exact-head review | RECONSTRUCTED / REVIEW PENDING | Historical 188-file selection → `e208a4c8`; main `0c26c159` reconciled; `e07d4542` source diff 194 files, +12,392/-4,510; local commits not pushed; three mobile workflows active |
...
| Staging gateway/realtime/voice | Gateway/provider/WSS/edge source and safe deployment | PRS OPEN / NOT LIVE | Infra #26 reviewed/green; gateway bootstrap local; realtime `4959d9e` has 2 P1 CR; OTP `29ff7af` and voice `6509c840` are test-green but CAS-race approval is superseded; binding two-phase rollout required |
| Twilio secret rotation | Jeeb-scoped restricted credential, consumers, canary, legacy revocation | IN PROGRESS / LIVE OTP BLOCKED | Protected `JEEB_*` installation fact only; owner-confirmed physical SMS canary recipient still absent; never record credential or recipient values |
| Scenario report reconciliation | `qa/scenarios/**` only | CURRENT THROUGH 17:22 UTC | Source `e07d4542`, 7,882/66/0 gate, credential/WSS closures, historical artifacts, cross-service CAS race, environment policy, binding rollout, blockers, and append-only corrections |
...
|---|---|---|---|---|
| STG-LIVE | Gateway, realtime, voice, OTP, edge, coordinator | Engine API version-CAS recovery implemented, tested, and approved across gateway/realtime/OTP/voice; Cloudflare token authority and physical SMS canary recipient ready | Binding two-phase gateway/realtime/edge sequence completes; fresh public probes close OTP, bypass/demo, voice, WSS, Firebase, AASA, and asset links; rollback proof retained | BLOCKED |
