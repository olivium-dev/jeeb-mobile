...
///
/// **MB1 W1.1 — this is the only position capability again.** A streaming
/// sibling briefly existed over the gateway's server-sent-events alias under
/// `/v1/geo/`. The gateway deleted that route (`LocationController.cs:22-31`)
/// 16 h after the mobile consumer landed, so every arm 404'd and the client
/// re-armed a dead GET forever. Both that interface and its Dio implementation
/// are deleted; `LiveTrackingCubit` reads this one-shot snapshot on screen-open,
/// on a status push, on resume and on retry — no cadence anywhere. The dead
/// symbols are deliberately not spelled out here: see
