...
/// the `order_summary_pinned` header renders (D11/D71). `LiveTrackingCubit`
/// reads it on screen-open, on a `type=delivery` push, on resume and on retry —
/// there is no cadence; when the status reaches the terminal delivered state the
/// screen auto-advances to the receipt prompt (JM-033).
class DioLiveTrackingRepository
  ...
  ///
  /// MB1 W1.1: this is the LIVE route and the only one. The gateway kept it
  /// (`LocationController.cs:227`) and deleted the `Accept: text/event-stream`
  /// arm together with the SSE alias that existed only to open it, because that
  /// arm was a 5 s server-side re-read loop — a poll wearing a stream's clothes
  /// (`LocationController.cs:22-31`). One read, one body, connection closed.
  ///
  ...
  Future<DeliveryLivePosition?> fetchLivePosition({
    ...
    try {
      ...
      // "Deliberately total" was a LIE until the MB1 two-model review, and it
      // was a lie in the one direction the doc comment above explicitly
      // promises to cover: a MALFORMED BODY. `fromTrackingJson` reaches the
      // wire through unchecked casts —
      ...
      // `DioException` nor a `FormatException`. It would have escaped an
      // unawaited call chain as an unhandled zone error, with no breadcrumb.
      //
