...
/// (`POST /v1/auth/logout`, `DELETE /api/PushNotification/device`) are
/// **best-effort**: a transport failure must NOT trap the user in a
/// logged-in-looking shell — the local keystore clear is the load-bearing step
/// and always runs.
abstract class AccountSessionTerminator {
  ...
  /// (`DELETE /api/PushNotification/device`), then ALWAYS clears the local
  /// keystore tokens. Never throws — a network failure still clears the local
  /// session (fail-safe: a half-logged-out state is worse than an offline
  /// logout).
  Future<void> logout();
