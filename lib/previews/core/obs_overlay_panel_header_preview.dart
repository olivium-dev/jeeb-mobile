...
import '../../core/observability/session_trace/presentation/widgets/obs_overlay_panel_header.dart';
import '../harness/jeeb_preview.dart';

/// The panel's own max width, restated from `_kMaxWidth` in
/// `obs_overlay_panel.dart`.
const double _kPanelMaxWidth = 340;
...
const Size _headerBox = Size(390, 160);
const Size _wrappingBox = Size(390, 200);
...
const double _overlayBoxHeight = 96;
...
double _panelWidth(double screenWidth) {
  ...
  return _kPanelMaxWidth < maxAvailable ? _kPanelMaxWidth : maxAvailable;
}
...
Widget _captioned(String caption, Widget framed) => Center(
      child: Column(
...
Widget _panelContent(double width) => SizedBox(
      width: width,
      child: Padding(
...
Widget _inPrivateOverlay(Widget child, {required double width}) => SizedBox(
      width: width,
      height: _overlayBoxHeight,
      child: Overlay(
...
/// guard for the "No Overlay widget found" crash (see [_inPrivateOverlay]):
...
Widget obsOverlayPanelHeaderInPanel() => _captioned(
      ...
      _inPrivateOverlay(
        _panelContent(_panelWidth(_phoneWidth)),
        width: _panelWidth(_phoneWidth),
      ),
    );
...
Widget obsOverlayPanelHeaderFixedHeightSlot() => _captioned(
      ...
      SizedBox(
        ...
        width: _panelWidth(_phoneWidth) - 2 * Spacing.medium,
        height: _fixedSlotHeight,
        child: ObsOverlayPanelHeader(
          controller: obsOverlayPanelHeaderPreviewController,
        ),
      ),
