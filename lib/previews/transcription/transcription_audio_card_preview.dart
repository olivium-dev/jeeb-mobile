...
/// recorder actually captured before confirming the request. `_TranscriptionBody`
/// renders it only when [TranscriptionState.hasAudio] is true, so every state
/// below carries a non-empty `audioPath`.
///
...
import '../../features/transcription/presentation/widgets/transcription_audio_card.dart';
...
/// box edge.
const Size _cardBox = Size(390, 160);

/// The gateway `audioId` every fixture carries — the Screen Catalog's id for
/// this screen. It is never rendered; it only makes [TranscriptionState.hasAudio]
/// true, which is what puts the card on screen at all.
const String _audioId = 'audio-ready-1';
...
const String _localPath = '/data/user/0/jeeb/cache/voice-request.m4a';

/// The Screen Catalog's fixture length for this screen.
const Duration _clipLength = Duration(seconds: 42);

/// `VoiceRecordingState.maxDuration` — the recorder's hard cap.
const Duration _recorderCap = Duration(seconds: 60);
...
Widget _hosted(TranscriptionState state, {bool bounded = false}) {
...
TranscriptionState _clip({
  required Duration total,
  Duration position = Duration.zero,
  bool isPlaying = false,
  String? localAudioPath = _localPath,
}) {
