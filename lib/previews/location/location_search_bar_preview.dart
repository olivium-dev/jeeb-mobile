...
///   then the draft card — using [_DraftCardStandIn], a fixture that is NOT
...
import '../../features/location/presentation/location_search_bar.dart';
import '../../l10n/app_localizations.dart';
import '../harness/jeeb_preview.dart';

/// A bar with no dropdown, plus a slice of the card below it.
const Size _barBox = Size(390, 240);

/// Room for a two-row dropdown.
const Size _shortListBox = Size(390, 320);

/// Room for the five-row dropdown plus the card it pushes down.
const Size _listBox = Size(390, 460);
...
class _DraftCardStandIn extends StatelessWidget {
  const _DraftCardStandIn({required this.fixture});
...
class _SearchBarHost extends StatefulWidget {
  const _SearchBarHost({
    super.key,
  ...
  @override
  State<_SearchBarHost> createState() => _SearchBarHostState();
}

class _SearchBarHostState extends State<_SearchBarHost> {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Expanded(child: _DraftCardStandIn(fixture: widget.fixture)),
        ],
      ),
    );
  }
...
const LocationPoint _downtown = LocationPoint(
  latitude: 33.8938,
  longitude: 35.5018,
  address: 'Downtown, Beirut',
);
const LocationPoint _hamra = LocationPoint(
  latitude: 33.8889,
  longitude: 35.4955,
  address: 'Hamra Street, Beirut',
);
const LocationPoint _gemmayze = LocationPoint(
  latitude: 33.8869,
  longitude: 35.5131,
  address: 'Gemmayze, Beirut',
);
const LocationPoint _achrafieh = LocationPoint(
  latitude: 33.8703,
  longitude: 35.5380,
  address: 'Achrafieh, Beirut',
);
const LocationPoint _verdun = LocationPoint(
  latitude: 33.9081,
  longitude: 35.4806,
...
/// first.
@JeebPreview(group: 'location', name: 'Idle · nothing typed', size: _barBox)
Widget locationSearchBarIdle() => _hosted(fixture: 'nothing typed');
...
@JeebPreview(group: 'location', name: 'Searching · nothing to list', size: _barBox)
Widget locationSearchBarSearching() => _hosted(
...
Widget locationSearchBarResults() => _hosted(
...
@JeebPreview(group: 'location', name: 'No matches', size: _barBox)
Widget locationSearchBarNoMatches() => _hosted(
      fixture: 'no matches',
      query: 'atlantis',
    );
...
Widget locationSearchBarJustSelected() => _hosted(
...
Widget locationSearchBarLongAndCoordinateOnly() => _hosted(
