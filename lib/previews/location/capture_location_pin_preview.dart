...
import '../../features/location/presentation/widgets/capture_location_pin.dart';
import '../../features/location/presentation/widgets/capture_map_viewport.dart';
...
const double _captureBodyHeight = 500;
...
const double _formBandHeight = Sizes.eightXLarge * 2;
...
const double _compactBandHeight = Sizes.fourXLarge;
...
const double _anchorBandHeight = 200;

/// Height of the neighbour rows in [captureLocationPinInlineOverdraw].
const double _neighbourRowHeight = Sizes.fourXLarge;

/// Half-length of each crosshair arm.
const double _crosshairArm = Sizes.twoXLarge;
...
Widget _measured(String caption, Widget scene) => Center(
      child: Column(
...
Widget captureLocationPinCaptureScreen() => _measured(
      'Capture screen: pin centred on a 390x500 map area',
      const SizedBox(
        key: hostKey,
        width: _phoneWidth,
        height: _captureBodyHeight,
        child: Stack(
          fit: StackFit.expand,
          children: <Widget>[
            CaptureMapViewport(),
...
@JeebPreview(
  ...
  size: Size(_phoneWidth, 260),
  matrix: true,
)
Widget captureLocationPinAnchorCrosshair() => _measured(
      'Anchor study: crosshair marks the returned coordinate',
      const SizedBox(
        key: hostKey,
        width: _phoneWidth,
        height: _anchorBandHeight,
        child: Stack(
          fit: StackFit.expand,
          children: <Widget>[
            CaptureMapViewport(),
            Center(child: _TargetCrosshair()),
            Center(child: CaptureLocationPin()),
          ],
        ),
      ),
...
Widget captureLocationPinAddressFormBand() => _measured(
      'Address form: 160pt clipped band',
      const SizedBox(
        key: hostKey,
        width: _phoneWidth,
        height: _formBandHeight,
        child: ClipRRect(
...
@JeebPreview(
  ...
  size: Size(_phoneWidth, 120),
)
Widget captureLocationPinCompactBand() => _measured(
...
Widget captureLocationPinInlineOverdraw() => _measured(
      ...
      const SizedBox(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: <Widget>[
            _NeighbourRow(label: 'Row above the pin', anchorKey: rowAboveKey),
            CaptureLocationPin(),
            _NeighbourRow(label: 'Row below the pin'),
          ],
        ),
      ),
    );
...
class _MapTileFill extends StatelessWidget {
  const _MapTileFill();
...
class _TargetCrosshair extends StatelessWidget {
  const _TargetCrosshair();

  @override
  Widget build(BuildContext context) {
    final Color ink = Theme.of(context).colorScheme.primary;
    return SizedBox(
      width: _crosshairArm * 2,
      height: _crosshairArm * 2,
      child: Stack(
...
class _NeighbourRow extends StatelessWidget {
  const _NeighbourRow({required this.label, this.anchorKey});
