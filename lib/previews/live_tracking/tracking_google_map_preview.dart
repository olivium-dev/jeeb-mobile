...
import '../../features/live_tracking/presentation/widgets/tracking_google_map.dart';
import '../harness/jeeb_preview.dart';

/// Phone width, and enough height for a map band plus the two caption lines.
const Size _mapBox = Size(390, 380);
...
const GpsPoint _pickup = GpsPoint(lat: 33.8938, lng: 35.5018);

/// The drop-off point of the same pilot route.
const GpsPoint _dropoff = GpsPoint(lat: 33.8869, lng: 35.5131);

/// The courier's reported fix (`trackingMarkers` / `trackingCamera` tests).
const GpsPoint _courier = GpsPoint(lat: 33.89, lng: 35.50);
...
const List<GpsPoint> _route = <GpsPoint>[_pickup, _dropoff];
...
String _readout(DeliveryTrackingInfo info) {
...
Widget _hosted(String label, DeliveryTrackingInfo info) => Builder(
...
Widget trackingGoogleMapSingleWaypoint() => _hosted(
      ...
      _info(
        polyline: const <GpsPoint>[_dropoff],
        status: PositionFreshness.awaitingFirstFix,
      ),
    );
