/// Widget previews for [ClientHomeEmptyView] — run with
...
import '../../features/home_client/presentation/widgets/client_home_empty_view.dart';
...
const double _requestsSlotHeight = 547;

/// The same arithmetic on a 320 × 568 phone: 568 − 20 − 56 = 492 pt of
/// viewport, minus the same 160 pt of chrome.
const double _compactSlotHeight = 332;

/// Canvas boxes: the framed slot plus a line for the caption.
const Size _phoneBox = Size(_phoneWidth, _requestsSlotHeight + 40);
const Size _compactBox = Size(_compactPhoneWidth, _compactSlotHeight + 40);
...
Widget _hosted({
  ...
  double slotHeight = _requestsSlotHeight,
  double? textScale,
  bool wired = true,
}) {
...
///
/// Preview scaffolding — see the library doc. Deliberately small and 1×-only so
/// the `EN 200% text` rendering of the matrix still shows the widget rather
...
class _FoldFrame extends StatelessWidget {
  const _FoldFrame({required this.child});
