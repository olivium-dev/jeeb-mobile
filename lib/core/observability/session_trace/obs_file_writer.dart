...
final class ObsFileWriter implements ObservabilitySink {
  ...
  static Future<ObsFileWriter?> installAsGlobal({
    ...
    try {
      ...
      if (path == null || writer.isBroken || !await File(path).exists()) {
        return null;
      }
      active = writer;
      return writer;
  ...
  void _addMetaLine(String name, Map<String, Object?> data) {
    ...
    _buffer.add(
      jsonEncode(<String, Object?>{
        'type': 'meta',
        'name': name,
        'data': SecretRedactor.redactBody(data),
  ...
  void _headerLine() {
    final redactedMeta = SecretRedactor.redactBody(_sessionMeta);
    final header = <String, Object?>{
      if (redactedMeta is Map<String, Object?>) ...redactedMeta,
      'type': 'session',
      'v': ObsEvent.schemaVersion,
    ...
    _buffer.insert(0, jsonEncode(SecretRedactor.redactBody(header)));
  }
