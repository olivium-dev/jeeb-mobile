...
**There is no GET on `/register`.** Device rows therefore cannot be enumerated, snapshotted, or verified after deletion — and MSI has no `psql`, and direct access to the `.20` DB box is banned by standing policy. DB name is `jeeb-push-notifications` (Jeeb-scoped data on the shared `.20` host), so a delete cannot reach a sibling product — but it can reach any Jeeb user's rows, invisibly.
