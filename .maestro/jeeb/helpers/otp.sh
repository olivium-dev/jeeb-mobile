...
#
# Prints ONLY the 4-digit code to stdout so it can be captured into a Maestro
# -e OTP=... injection. Exits non-zero if no code is found.
#
# Requires psql on PATH and network line-of-sight to the DB host (the Mac has it
# via the .2.0/24 LAN; a device does not need it).
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
