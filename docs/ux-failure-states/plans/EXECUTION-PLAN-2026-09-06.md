...
## 0. What moved since the plans were written (verified live, 07:06–07:10 UTC)
...
|---|---|---|---|---|---|---|
| **P01** dm-onboarding route | OD-1 **Other** ("keep the formbuilder, store data in user preferences through the gateway"); OD-2 **nobbox**; OD-3 combined | **PARKED until v2 is approved.** G1–G12 (new `/v1/jeebers/me/onboarding` BFF), M1–M5, §7A curl proof and §7B device run are all withheld; the `p01-onboarding` worktree is NOT created; the `fix/dm-onboarding-route` mobile branch is NOT opened. Under OD-2 the v2 ships no Production bbox (mechanism only, if v2 keeps a coverage check at all). The combined deploy (OD-3) proceeds with P02-G + P03-G only; if v2 lands later it needs its own owner deploy. | — (parked) | — (none created) | — (removed from the queue) | **parked** |
...
| **P10** PR readiness | OD-12 **raise35**; OD-13 **squash + keep branch**; OD-14 **nextday**; OD-15 **undecided**; OD-0 widen | §2 timeout commit rides batch 0 (applies to its own run). C1 scope freeze VOID → R4 re-smoke happens on **three** head SHAs (batch 0/1/2 heads), not one. §5 lane reviews become delta reviews per batch + a final pass on the batch-2 head. §6 ADR-0004 (with the C3 R6 refinement) + reviewer map + migration notes ride batch 2. §7.2 RC/distribute preparation for the merge SHA `M`, day-after-merge date codes `26MMDD01` (Android) / `26MMDD02` (iOS), both > `26090403` / `26090402`. **D4 (`epic/wallet-guard-fix` re-baseline) PARKED** pending OD-15. Two new red contexts (L2/L3) are added to §1's readiness definition. | #335: batch 0 (CI), batch 2 (ADR/docs) | — | 1, 3, 12 (smokes) | **ready** (D4 parked) |
...
Totals: **12 ready, 1 parked (P01), 0 dropped.** Dropped sub-items: P07-F4, P07-F8, P02-M3, P08-S17, the C1 scope
freeze (void). Parked sub-item: P10-D4 (OD-15).
...
|---|---|---|---|
| **0** (today) | 1 P12-A · 2 P11 C1+C2 · 3 P10 §2 timeout 20→35 · 4 N2 docs reword (`PLAN-P10-pr-readiness.md:171`) · (5 N1 rubyzip ignore, only if the owner answers before the push) | P10 §4 local gate (~25 min) | `verify` green; stage cap 35 applies to this run; P10 §7.1 smoke + P11 proof (wave 1); batch-1 lanes rebase onto this head |
| **1** | 6 P08 D1–D6 · 7 P04-B · 8 **P13** · 9 **P05** · 10 **P06** · 11 **P07** · 12 **P02-M c1** · 13 **P03-M** · (N1 if not in batch 0) | local gate + P05 WI-8 step 5 devtool-define suite + `bash tool/test_fault_proxy.sh` + `bash tool/test_device_validation_cleanup.sh` | the wave-3 device queue runs on ONE build (this head) |
...
2. **Lanes A/B/C — batch 0 code, three disjoint fences, parallel, then integrated in order A → B → C:**
   ...
   - **Lane C (CI + red contexts):** `.github/workflows/ci-flutter-stage.yml:55` (20→35);
     ...
     it prints nothing); `.github/workflows/release-security.yml:65` ONLY if N1 is answered before the push.
...
| **3b** | P12-B (after S1.6 captured) + defect fixes + ADR-0004/reviewer map/migration notes → batch-2 push → CI → re-smoke (slot 12, incl. P12 §7.7) → lane reviews final pass (all 11 lanes, delta + head) → `gh pr ready 335` → **owner squash-merges, keeps the branch (OD-13)**. | all 5 required contexts green on the batch-2 head — this needs **N1** answered; every lane review PASS; `gh pr ready` is ours, the merge is the owner's. |
| **4** | Owner merges P02-G + P03-G → **ONE MSI deploy + ONE staging dispatch (OD-3, owner-executed)** → P02 wire proof (`jq '.items[] | {type, ref, deepLink, ts}'`) + commit 2 (batch 3 or post-merge PR) + V1–V12 (slot 13) → P03 Phase B (slot 13). P01 §7A/§7B do NOT run (parked). | owner: two gateway merges + one deploy pair. If P01 v2 is approved before the deploy, it can join; otherwise it needs a second owner deploy later. |
| **5** | Day after the merge (OD-14): owner dispatches Android RC + distribute (`26MMDD01`) and iOS RC + distribute (`26MMDD02`) for the merge SHA `M`; exact commands with the real `M` posted in the PR's final comment by us (P10 §7.2). `epic/wallet-guard-fix` re-baseline: **PARKED (OD-15)**. | owner: 4 workflow dispatches; OD-15 decision. |

**Parked:** P01 (entire plan) until the v2 is approved (owner item 1 in §4); P10-D4 / wave-5 epic re-baseline until OD-15 is answered.
...
rubyzip advisory needs your call (N1) and blocks the merge of every PR to `main`, not only #335.
...
the P01 v2 keeps a coverage check at all: the `ZoneBoundary`-typed options section + resolver, the 409 problem type
`https://jeeb.dev/errors/out_of_coverage`, the two gwdbx registry rows, and mobile's `out_of_coverage` discriminator
(tested but dead on live). Because OD-1 parked P01, this only takes effect when v2 ships.
...
1. **OD-1 v2 confirmation (placeholder — fill in when the v2 draft exists).** Proposed question: *"P01 v2: mobile keeps
   posting the form-builder template `jeeb_jeeber_v1` to `<path — the live `POST /form-builder/forms/{templateName}`
   route, or the mobile's `/form-builder-service/v1/templates/{t}/submit` as a new alias?>`; the gateway persists the
   submission as one remote-user-preferences blob (`jeeb.jeeber_onboarding` or a template-keyed key) under capability
   `<kyc.submit.self | profile.write.self | other>`; coverage = mechanism only, no Production bbox (OD-2); the mobile
   fail-safe (silent no-op on 404/405/501) is removed only after that route is deployed on MSI + staging — confirm
   yes/no and give the path + capability."* Note for the v2 planner: `jeeb_jeeber_v1` is the KYC template on the real
   gateway (P01 E5) and the live `POST /form-builder/forms/jeeb_jeeber_v1` probe returned a Cloudflare 502 (P01 side
   observation) — both must be resolved in v2. Until answered: P01 parked, no worktree, no PR, no queue slot.
2. **OD-15 decision, after this explanation.** *Which one is used now:* what testers and the dev gateway run is
   ...
   *Is there a breaking change:* the re-baseline itself changes nothing users run — it is a branch operation; the epic's
   ...
   `insufficient_balance` through `GatewayProblem.reasonCode`. Please answer `rebaseline` / `mergemain`.
3. **N1 (new, urgent — blocks every merge to `main`):** approve `bundle-audit check --update --ignore CVE-2026-85396`
   in `.github/workflows/release-security.yml:65` with a dated comment and the removal condition "first fastlane release
   whose gemspec admits rubyzip ≥ 3.4.0" (fastlane 2.238.0 and 2.239.0 both pin `< 3.0.0`; rubyzip's fix is 3.4.0, latest
   3.6.0). This weakens a release gate for one CVE in the release toolchain, so it is yours to grant. Yes → it rides batch
   0 if answered before the push, else batch 1.
...
6. **Owner-executed steps, in order:** (a) merge gateway PRs P02-G and P03-G when green; (b) run ONE MSI deploy + ONE
   staging dispatch (`jeeb-staging-deploy.yml`, `deployment_mode=normal`) for the merged gateway `main` (OD-3) — P01 v2
   joins only if approved before this; (c) after `gh pr ready 335` and the final lane reviews: squash-merge #335, keep
