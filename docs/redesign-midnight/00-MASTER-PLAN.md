...
| **M5 Motion pass** | Wire §2.6 primitives per live-board observation (R2 waveform+halo, R1 rings+broadcast dot, transcript caret, map `jDash`, walkthrough floats); audit pass-1 Lotties on navy (recolor/replace/retire); reduced-motion verified | Opus, Fable arbitrates taste | motion matches the live board side-by-side |
...
- [ ] M0-7 Board assets imported (`delivery-3d.png`, `jeeb-wordmark.svg`)
