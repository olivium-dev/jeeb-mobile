...
47. [Q-047] (M6) **2 raw hex remain a sanctioned exemption, not a defect** — JEEB-57's Google slice now uses a vetted four-colour SVG asset; `social_sign_in_button.dart` retains only Facebook Brand Blue and one shared white backing/glyph literal for Google, Facebook, and Apple. The M6 gate asserts "≤2 hex outside theme, all in that file" rather than a plain zero.
