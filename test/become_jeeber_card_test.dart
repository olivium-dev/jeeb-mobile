...
void main() {
  testWidgets(
    'AC1: card is visible for Client user without Jeeber role',
    (tester) async {
      await tester.pumpWidget(
        _host(isAlreadyJeeber: false),
      );
  ...
  testWidgets(
    'AC2: card is hidden when user already has Jeeber role',
    (tester) async {
      await tester.pumpWidget(
        _host(isAlreadyJeeber: true),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'AC3: tapping the CTA calls onTap callback',
    (tester) async {
      var tapped = false;
      await tester.pumpWidget(
  ...
  testWidgets(
    'AC4: card has semantic label for screen readers',
    (tester) async {
      await tester.pumpWidget(_host(isAlreadyJeeber: false));
      await tester.pumpAndSettle();

      final semantics = tester.getSemantics(
        find.byKey(BecomeJeeberCard.rootKey),
      );
      expect(
        semantics.label,
        contains('Jeeber'),
