...
final DateTime _controlledNow = DateTime(2026, 5, 17, 12);
...
DeliveryRequest _req({
  ...
  final clock = now ?? _controlledNow;
...
void main() {
    ...
    blocTest<RequestFeedCubit, RequestFeedState>(
      ...
      expect: () => [
        predicate<RequestFeedState>(
          (s) => s.status == RequestFeedStatus.loading,
        ),
        predicate<RequestFeedState>(
      ...
      build: () {
        ...
        addTearDown(() {
          expect(
            sounds,
            1,
            reason: 'sound should fire once per new request id',
          );
        });
        return cubit;
      },
  ...
  group('snapshot-authoritative reconcile (Lane C)', () {
    test('refresh drops a stale row the gateway no longer lists', () async {
      ...
      await cubit.refresh();
      expect(
        cubit.state.requests.map((r) => r.id),
        ['a2'],
    ...
    test(
      'refresh preserves an existing row with an action in flight',
      () async {
        ...
        await Future<void>.delayed(const Duration(milliseconds: 10));
        expect(
          cubit.state.actionStatusFor('r1'),
          RequestActionStatus.accepting,
        );
        await cubit.refresh();
        expect(
          cubit.state.requests.map((r) => r.id),
  ...
  group('honest lifetime — server expiresAt is the ONLY deadline (G3)', () {
    ...
    test('expiry sweep pauses while hidden and reconciles immediately on '
      ...
      expect(
        ...
        isEmpty,
        reason: 'tickOnResume removes elapsed rows without waiting a period',
      );
      expect(cubit.state.isExpired('r1'), isFalse);
      await cubit.close();
    });
    ...
    test('card SURVIVES past the old 60s client cap and stays live until '
      ...
      await Future<void>.delayed(const Duration(milliseconds: 80));
      expect(
        cubit.state.requests.length,
        1,
        reason:
            'the old min(expiresAt, +60s) cap retired this card at 60s '
    ...
    test(
      'past expiresAt the card is removed without an expired ghost state',
      () async {
        var now = DateTime(2026, 5, 17, 12);
        when(() => repo.refresh()).thenAnswer(
          (_) async => [
    ...
    test(
      'accept/decline on an expired card is a no-op (dead server-side)',
      () async {
        var now = DateTime(2026, 5, 17, 12);
        when(() => repo.refresh()).thenAnswer(
          (_) async => [
        ...
        await cubit.start();
        now = now.add(const Duration(seconds: 6));
        // Tap before the periodic sweep: the action guard owns this race.
    ...
    test(
      'refresh does not preserve an elapsed card omitted by the snapshot',
      () async {
        var now = DateTime(2026, 5, 17, 12);
        final snapshots = <List<DeliveryRequest>>[
          [_req(id: 'r1', ttl: const Duration(seconds: 5), now: now)],
    ...
    test(
      'an already-elapsed snapshot row is filtered before first render',
      () async {
        final now = DateTime(2026, 5, 17, 12);
        when(() => repo.refresh()).thenAnswer(
          (_) async => [
            _req(id: 'r1', ttl: const Duration(seconds: -1), now: now),
          ],
        );
        final cubit = build(clock: () => now);
        ...
        expect(cubit.state.status, RequestFeedStatus.ready);
        expect(cubit.state.requests, isEmpty);
        expect(cubit.state.isExpired('r1'), isFalse);
        await cubit.close();
      },
    ...
    test(
      'a re-listed id revives from the expired state (server disagrees)',
      () async {
        var now = DateTime(2026, 5, 17, 12);
