...
void main() {
  ...
  group('RecordVoiceRecorder (T-MOB-011)', () {
    ...
    test(
      'start throws permissionDenied when the mic permission is refused',
      () async {
    ...
    test('start asks for permission then records to a temp m4a path', () async {
    ...
    test(
      'stop reads the recorded bytes back into a VoiceClip with sourcePath',
      () async {
    ...
    test(
      'stop throws unknown when the platform yields an empty buffer',
      () async {
        when(() => platform.hasPermission()).thenAnswer((_) async => true);
        when(
          () => platform.start(any(), path: any(named: 'path')),
    ...
    test('stop throws unknown when the platform returns a null path', () async {
      when(() => platform.hasPermission()).thenAnswer((_) async => true);
      when(
        () => platform.start(any(), path: any(named: 'path')),
      ).thenAnswer((_) async {});
    ...
    test(
      'cancel aborts the platform recorder and deletes the temp file',
      () async {
    ...
    test(
      'maps an unavailable-flavoured plugin error onto unavailable',
      () async {
        when(() => platform.hasPermission()).thenAnswer((_) async => true);
        when(
          () => platform.start(any(), path: any(named: 'path')),
  ...
  group('AudioPlayersVoicePlayer (T-MOB-011)', () {
    ...
    test(
      'play streams a DeviceFileSource when the clip has a sourcePath',
      () async {
    ...
    test(
      'play falls back to BytesSource when no sourcePath is present',
      () async {
        final player = AudioPlayersVoicePlayer(player: platform);

        await player.play(clip(), onPosition: (_) {}, onCompleted: () {});
    ...
    test(
      'play bridges position ticks and the completion event to callbacks',
      () async {
        final player = AudioPlayersVoicePlayer(player: platform);
    ...
    test(
      'stop cancels the subscriptions so no further ticks reach callbacks',
      () async {
        final player = AudioPlayersVoicePlayer(player: platform);
        final positions = <Duration>[];
    ...
    test(
      'pause delegates to the platform player without tearing down streams',
      () async {
        final player = AudioPlayersVoicePlayer(player: platform);
        final positions = <Duration>[];
    ...
    test(
      'seek delegates the requested playhead to the platform player',
      () async {
        final player = AudioPlayersVoicePlayer(player: platform);
        ...
        verify(
          () => platform.seek(const Duration(milliseconds: 1250)),
        ).called(1);
      },
    );
  });
