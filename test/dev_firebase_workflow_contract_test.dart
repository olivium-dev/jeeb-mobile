...
const _protectedInjectionSecret =
    'secrets.JEEB_DEVELOPMENT_JEEB_MOBILE_ANDROID_GOOGLE_SERVICES_JSON_B64';
...
const _secretControlledProject = 'DEV_FIREBASE_EXPECTED_PROJECT_';
...
void main() {
  group('workflow re-enable readiness (Phase P depends on these surviving)', () {
    test('Analyze and sharded coverage keep strict, non-duplicated gates', () {
    ...
    test('Flutter, Android, coverage, and mobile stages keep the pin gate', () {
    ...
    test('Android stage owns the protected dev config build', () {
      for (final path in const <String>[
        ...
        expect(
          workflow,
          isNot(contains('secrets.DEV_GOOGLE_SERVICES_JSON_B64')),
        );
        expect(
          workflow,
    ...
    test('iOS dev build receives its development organization config', () {
      final workflow = File(
      ...
      expect(
        workflow,
        contains(
          'secrets.JEEB_DEVELOPMENT_JEEB_MOBILE_IOS_GOOGLE_SERVICE_INFO_PLIST_B64',
        ),
      );
      expect(workflow, contains('tool/run_with_ios_dev_firebase_config.sh'));
      expect(
        workflow,
        isNot(contains('secrets.IOS_DEV_GOOGLE_SERVICE_INFO_PLIST_B64')),
      );
    });
