...
void main() {
  ...
  group(
    'endpoint contract (regression — must target the request-centric feed)',
    () {
      test(
        'refresh() GETs /v1/jeebers/me/feed?status=pending, NOT /requests',
        () async {
      ...
      test(
        'empty/offline feed → no requests (200 {items:[],totalCount:0})',
        () async {
          dio.nextResponse = _resp({'items': <dynamic>[], 'totalCount': 0});
  ...
  group('JeeberFeedItem parse alignment (frozen Contract B shape)', () {
    test(
      'keys the item by requestId and maps description + tierId + location',
      () async {
    ...
    test(
      'TX.1 — offerDeadlineInSeconds anchors against the injected clock',
      () async {
    ...
    test(
      'TX.1b — elapsed rows are omitted and diagnosed with context',
      () async {
        final t0 = DateTime.utc(2026, 6, 30, 9, 41);
        final anchored = DioRequestFeedRepository(dio: dio, now: () => t0);
        addTearDown(anchored.dispose);
          ...
          'items': [
            {
              'requestId': 'req-zero',
              'status': 'pending',
              'offerDeadlineInSeconds': 0,
            },
            {
              'requestId': 'req-negative',
              'status': 'pending',
              'offerDeadlineInSeconds': -12,
            },
            {
              'requestId': 'req-open',
              'status': 'pending',
              'offerDeadlineInSeconds': 1,
            },
          ],
          'totalCount': 3,
        });
        ...
        expect(result.map((request) => request.id), ['req-open']);
        expect(result.single.expiresAt, t0.add(const Duration(seconds: 1)));
      },
    );

    test(
      'TX.2 — a legacy absolute is NOT honoured (the dual-read is gone)',
      () async {
        dio.nextResponse = _resp({
          'items': [
            {
    ...
    test(
      'TX.3 — no deadline field at all: null deadline, card stays live',
      () async {
        dio.nextResponse = _resp({
          'items': [
            {
    ...
    test(
      'terminal request status is preserved as non-actionable authority',
      () async {
        dio.nextResponse = _resp({
          'items': [
            {
    ...
    test(
      'degrade-don\'t-drop: item with no pickup/dropoff still renders',
      () async {
