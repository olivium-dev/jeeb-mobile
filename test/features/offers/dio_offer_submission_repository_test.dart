...
void main() {
  ...
  group('submitOffer — route + body (BUG-2: the live offer route)', () {
    test(
      'POSTs to the live request-scoped route /requests/{id}/offers '
  ...
  group('submitOffer — response parse', () {
    ...
    test(
      'falls back to the requestId for conversationId when the live 201 '
    ...
    test(
      'prefers an explicit conversationId if the gateway ever returns one',
      () async {
    ...
    test('tolerates the legacy `offerId` key', () async {
      ...
      ).thenAnswer((_) async => _resp(const {'offerId': 'legacy-offer-1'}));
  ...
  group('submitOffer — error mapping', () {
    ...
    test('bare 409 (no discriminator) → conflict, not requestGone', () async {
      when(
        () => dio.post<Map<String, dynamic>>(
          any(),
          data: any(named: 'data'),
    ...
    test('409 request-not-open-for-offers → requestNotOpen (bounce to feed), '
        'NOT the cap', () async {
      when(
        () => dio.post<Map<String, dynamic>>(
          any(),
    ...
    test(
      'AE-05/UX-12: an undiscriminated 409 is a conflict, never expiry',
      () async {
        when(
          () => dio.post<Map<String, dynamic>>(
            any(),
    ...
    test('409 offer-submit-conflict remains non-terminal', () async {
      when(
        () => dio.post<Map<String, dynamic>>(
          any(),
          data: any(named: 'data'),
      ...
      await expectLater(
        ...
        throwsA(
          isA<OfferSubmissionException>()
              ...
              .having((e) => e.cause, 'cause', isA<ConflictFailure>()),
        ),
      );
    });
    ...
    test('402 → insufficientBalance with parsed amounts', () async {
      when(
        ...
        _err(402, body: {'needed': 0.5, 'available': 0.2, 'currency': 'USD'}),
      );
      ...
      await expectLater(
        ...
        throwsA(
          isA<OfferSubmissionException>()
              ...
              .having((e) => e.toString(), 'toString', isNot(contains('HTTP'))),
        ),
      );
    });
