...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      // First tap: the dead no-op is gone — the button opens the picker seeded
      // with the form's current centre (the add-path Beirut fallback).
      await tester.tap(find.text('Edit pin'));
      await tester.pumpAndSettle();
      expect(launcher.calls, 1);
      expect(launcher.lastInitial?.latitude, 33.8886);
      expect(launcher.lastInitial?.longitude, 35.4955);
