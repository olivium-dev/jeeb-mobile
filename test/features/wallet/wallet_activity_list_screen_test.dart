...
class _ScriptedLedgerRepository implements WalletLedgerRepository {
  _ScriptedLedgerRepository({List<WalletLedgerEntry>? entries, this.throws})
...
void main() {
  ...
  testWidgets('AC: root + typed rows render with the per-row id', (
    tester,
  ) async {
    await pump(
      tester,
    ...
    expect(
      find.bySemanticsIdentifier('wallet_activity_row_led-b'),
      findsOneWidget,
    );
    expect(
      find.bySemanticsIdentifier('wallet_activity_row_led-c'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('D30 error: cold-load failure → error + retry_cta', (
    tester,
  ) async {
    await pump(
      tester,
  ...
  testWidgets('a 401 gets the way out, never a headline with no act', (
    ...
    await pump(
      tester,
      _ScriptedLedgerRepository(throws: WalletLedgerFailure.unauthorized),
    );
    ...
    expect(
      find.bySemanticsIdentifier('wallet_activity_exit_cta'),
      findsOneWidget,
    );
  });
