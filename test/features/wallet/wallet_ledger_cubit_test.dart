...
class _PagedRepository implements WalletLedgerRepository {
  ...
  Future<WalletLedgerPage> fetchLedger({int page = 1, int pageSize = 20}) async {
    ...
    final end = (start + pageSize) > _all.length ? _all.length : start + pageSize;
    final slice = (start >= _all.length)
        ? const <WalletLedgerEntry>[]
        : _all.sublist(start, end);
    final totalPages = _all.isEmpty ? 1 : ((_all.length + pageSize - 1) ~/ pageSize);
...
void main() {
  group('WalletLedgerCubit.load', () {
    test('initial → loaded with page 1 + hasMore when more pages exist',
        () async {
    ...
    test('single page → loaded, hasMore false', () async {
      final cubit =
          WalletLedgerCubit(repository: _PagedRepository(all: _rows(5)), pageSize: 20);
      await cubit.load();
    ...
    test('network failure → failed + typed WalletLedgerFailure.network',
        () async {
      final cubit = WalletLedgerCubit(
        repository: _PagedRepository(fetchThrows: WalletLedgerFailure.network),
      );
      await cubit.load();
  ...
  group('WalletLedgerCubit.loadMore (infinite scroll)', () {
    ...
    test('load-more failure → soft loadMoreError, rows intact; retry recovers',
        () async {
