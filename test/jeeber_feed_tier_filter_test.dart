...
void main() {
  group('JeeberFeedTabView – tier filter chips', () {
    testWidgets('AC1: tier filter strip renders when Jeeber is online', (
      tester,
    ) async {
    ...
    testWidgets('AC2: Flash chip activates and filters requests by tier', (
      tester,
    ) async {
      final ticker = StreamController<DateTime>.broadcast();
      addTearDown(ticker.close);
    ...
    testWidgets('tier filter strip is hidden when Jeeber is offline', (
      tester,
    ) async {
      final avCubit = AvailabilityCubit(
        gateway: InMemoryAvailabilityGateway(
